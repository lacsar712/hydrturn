...
func (s *SteamPath) ComputeFlow(snap model.PlantSnapshot, firing bool) float64 {
	...
	loadFactor := snap.Turbineln.OutputMW / math.Max(snap.Settings.TargetMW, 1)
	pressureFactor := snap.Turbineln.SteamPressurePSI / math.Max(snap.Settings.TargetSteamPSI, 1)
...
func (s *SteamPath) ComputeMW(snap model.PlantSnapshot, firing bool) float64 {
	...
	enthalpy := s.enthalpyFromPressure(snap.Turbineln.SteamPressurePSI)
...
func (s *SteamPath) EnthalpyBalance(snap model.PlantSnapshot) float64 {
	inEnergy := snap.Draftube.FeedwaterTPH * 200
	outEnergy := snap.Draftube.SteamFlowTPH * s.enthalpyFromPressure(snap.Turbineln.SteamPressurePSI)
...
func (s *SteamPath) LoadPercent(snap model.PlantSnapshot) float64 {
	...
	return (snap.Turbineln.OutputMW / snap.Settings.TargetMW) * 100
}
