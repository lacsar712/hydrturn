...
func (c *Controller) Tick(ctx context.Context, snap model.PlantSnapshot, firing bool) (model.TurbinelnReading, error) {
	select {
	case <-ctx.Done():
		return snap.Turbineln, fmt.Errorf("%w", model.ErrContextDone)
	default:
	}
	pressure, err := c.pressure.Compute(snap, firing)
	if err != nil {
		return snap.Turbineln, err
	}
