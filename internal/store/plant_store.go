...
func (s *PlantStore) UpdateDraftube(unitID string, draftube model.DraftubeReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateWicket(unitID string, comb model.WicketReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
...
func (s *PlantStore) UpdateTurbineln(unitID string, turbineln model.TurbinelnReading) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	snap, ok := s.units[unitID]
	if !ok {
