...
// HublockDrivePulse counts hublock-side effects from FSM after hooks (acceptance).
var HublockDrivePulse func()

func RegisterHublockDriveHook(chain *HookChain) {
	chain.OnAfter(func(ctx context.Context, from, to model.PlantState, event PlantEvent) error {
		_ = ctx
		_ = from
		_ = to
		_ = event
		if HublockDrivePulse != nil {
			HublockDrivePulse()
		}
		return nil
	})
