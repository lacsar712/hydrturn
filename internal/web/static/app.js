...
function renderSnapshot(snap) {
  ...
  document.getElementById("pressure").textContent = snap.turbineln.steam_pressure_psi.toFixed(0);
  document.getElementById("mw").textContent = snap.turbineln.output_mw.toFixed(1);
  document.getElementById("draftube-level").textContent = snap.draftube.level_percent.toFixed(1);
  document.getElementById("burner").textContent = snap.wicket.burner_phase;
  document.getElementById("o2").textContent = snap.wicket.excess_o2_pct.toFixed(2);
  trendData.push(snap.turbineln.steam_pressure_psi);
...
document.getElementById("btn-sync").addEventListener("click", () => postAction("/sync/start?holder=hmi"));
document.getElementById("btn-sync-done").addEventListener("click", () => postAction("/sync/complete?holder=hmi"));
